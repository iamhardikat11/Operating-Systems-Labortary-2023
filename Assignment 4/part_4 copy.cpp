...
void *readPost(void *arg){
    while(1){
        // LOCK(&mutex);
        if(queue_moniter.empty()){
            // UNLOCK(&mutex);
            continue;
        }
        Action a = queue_moniter.front();
        queue_moniter.pop_front();
        // UNLOCK(&mutex);
        print_message(a.action_id, a.action_type, a.user_id, a.timestamp);
    }
}
...
signed main()
{
    ...
    //     create threads for reading the posts
    pthread_t readPost_thread[10];
    for(int i = 0; i < 10; i++){
        int ret =  pthread_create(&readPost_thread[i], NULL, readPost, NULL);
        if(ret != 0) {
                printf("Error: pthread_create() failed\n");
                exit(EXIT_FAILURE);
        }
